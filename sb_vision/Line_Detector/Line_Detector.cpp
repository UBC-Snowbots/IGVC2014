...
int detectTransitions(int);

Mat image, image_grey, image_filter, image_thresholded, image_canny, image_blur, image_blur2, image_direction;
Mat image_H, image_S, image_V, image_histo,image_HThresh,image_SThresh,image_VThresh;
...
int main(int argc, char** argv) {
	...
	while (1) {
		...
		if (waitKey(1) == 27)break; // Wait for one ms, break if escape is pressed
	}
	destroyAllWindows();
	return 0;
}
...
void filterImage(void) {
	...
	//Blur Image
	if (blur_value % 2 == 0)blur_value = blur_value + 1;
	...
	//Regular threshold
	threshold(image_blur2, image_thresholded, threshold_value, max_BINARY_value, THRESH_BINARY);
	//Adaptive threshold
...
void getDirection(void) {
	...
	{
		//transitions =  detectTransitions(startRow);
		transitions = countLines(row);
		drawLine(row);
...
int detectTransitions(int row)
{
	...
	for (int i = 0; i < image_thresholded.cols; i++) {
		currentValue = (image_thresholded.at<uchar>(row, i)) % 2; // had problems with data type of binary image. Modulo works to get either 1 or 0
...
void displayWindows(void) {
	...
	cvMoveWindow("Hue Otsu Threshold", 400,600);
